...
import { getAllBoardData } from "../src/sqlQueries";

export const getAllBoardDataForUserById = async (
  req: Request,
  res: Response
) => {
  ...
  if (getUserById.rows.length > 0) {
    ...
    if (getBoardById.rows.length > 0) {
      try {
        const dbRes = await client.query(getAllBoardData, [user_id, board_id]);
